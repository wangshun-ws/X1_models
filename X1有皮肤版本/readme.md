...
下肢关节的力矩范围如下，来源于 `X1.urdf` 中各关节的 `effort` 值，与 `X1.xml` 中的 `forcerange` 一致：

| 关节名                                                  | 力矩范围            |
| ---------------------------------------------------- | ------------------- |
| `left_hip_yaw_joint` / `right_hip_yaw_joint`         | `[-50, 50]`         |
| `left_hip_roll_joint` / `right_hip_roll_joint`       | `[-100, 100]`       |
| `left_hip_pitch_joint` / `right_hip_pitch_joint`     | `[-212, 212]`       |
| `left_knee_joint` / `right_knee_joint`               | `[-93, 93]`         |
| `left_ankle_pitch_joint` / `right_ankle_pitch_joint` | `[-64.8, 64.8]`     |
| `left_ankle_roll_joint` / `right_ankle_roll_joint`   | `[-20, 20]`         |
